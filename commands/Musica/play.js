...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!permissions.has("CONNECT")) return sendError("Eu não consigo entrar no canal, verifique minhas permissões!", message.channel);
        if (!permissions.has("SPEAK")) return sendError("Eu não consigo falar no canal, verifique minhas permissões!", message.channel);

        var searchString = args.join(" ");
        if (!searchString) return sendError("Ok!", message.channel);
        ...
        if (url.match(/^(https?:\/\/)?(www\.)?(m\.)?(youtube\.com|youtu\.?be)\/.+$/gi)) {
            try {
                ...
                if (!songInfo) return sendError("Não encontrei nada como esse título no YouTube.", message.channel);
                song = {
            ...
            try {
                ...
                if (!songInfo) return sendError("Não encontrei nada como esse título no SoundCloud.", message.channel);
                song = {
            ...
            try {
                ...
                if (searched.videos.length === 0) return sendError("Não consegui encontrar nada no YouTube.", message.channel);
        ...
        if (serverQueue) {
            ...
            let thing = new MessageEmbed()
                .setAuthor("Música adicionada à fila.")
                .setThumbnail(song.img)
                .setColor("YELLOW")
                .addField("Nome:", song.title, true)
                .addField("Duração:", song.duration, true)
                .addField("Adicionada por:", song.req.tag, true)
        ...
        const play = async (song) => {
            ...
            if (!song) {
                sendError(
                    "Saindo do canal pois não há nada para tocar.",
                    message.channel
                );
                    ...
                    stream.on("error", function (er) {
                        if (er) {
                            if (queue) {
                                queue.songs.shift();
                                play(queue.songs[0]);
                                return sendError(`Um erro ocorreu.\nPossível tipo: \`${er}\``, message.channel);
                            }
                        }
                    });
                }
        ...
        try {
            ...
            message.client.queue.delete(message.guild.id);
            await channel.leave();
            return sendError(`Não consegui entrar no canal: ${error}`, message.channel);
        }
    },
};